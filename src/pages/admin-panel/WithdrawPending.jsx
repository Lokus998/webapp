...
function WithdrawPending() {
    ...
    const [withdrowData, setwithdrowData]=useState({withdrowData:[]});
...
{withdrowData.withdrowData.filter(data=>data.Type==='Withdrow'&&data.Status==='Pending').map((row, index) =>{
return(
  <tr key={index}>
  <td>{row.PaymentID}</td>
  <td>{row.UserName}</td>
  <td >{row.Amount}</td>
  <td  data-paymentid={row.PaymentID} onClick={acceptwithdraw}> ✔</td>
  <td  data-paymentid={row.PaymentID} onClick={canclwithdraw}>❌</td>
</tr>
)
})}
